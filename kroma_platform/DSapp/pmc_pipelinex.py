...
# Some PMC pages expose a PDF on the article webpage while EFetch/OA XML is
# abstract-only and the OA service may not list the PDF. Keep a small explicit
# override hook for these edge cases. Values can also be extended from Django
# settings.PMC_PDF_URL_OVERRIDES.
PMC_PDF_URL_OVERRIDES = {
    13081882: "https://pmc.ncbi.nlm.nih.gov/articles/PMC13081882/pdf/nihpp-2026.04.04.715761v1.pdf",
}
...
def get_pdf_url_override(pmcid: int) -> Optional[str]:
    """Return a manually configured PDF URL for known PMC edge cases."""
    configured = getattr(settings, "PMC_PDF_URL_OVERRIDES", None) or {}

    for source in (configured, PMC_PDF_URL_OVERRIDES):
        for key in (pmcid, str(pmcid), f"PMC{pmcid}"):
            url = source.get(key) if hasattr(source, "get") else None
            if url:
...
def download_pdf_if_available(pmcid: int) -> str:
    ...
    1) Explicit override for known PMC edge cases
    2) OA service PDF
    3) PMC article page PDF link
    ...
    override_url = get_pdf_url_override(pmcid)
    if override_url:
        candidate_sources.append(("override", override_url))
    ...
    try:
        candidate_sources.append(("article_page", get_pdf_url_from_article_page(pmcid)))
    except Exception as exc:
...
def ingest_article(
    ...
    # Use the best available text for classification:
    # 1) full-body XML, 2) PDF text, 3) weak abstract XML fallback.
    fulltext_text = ""
    ...
    if xml_is_usable_fulltext and xml_rel_path:
        ...
        effective_fulltext_path = xml_rel_path
        effective_fulltext_format = "abs_xml"
