...
# Preprints should not enter the KroMA/DS-KG ingestion set because they are
# not peer-reviewed. Keep the spellings broad because users and metadata may
# contain either x/y variants or common typos.
PREPRINT_SOURCE_PATTERNS = [
    "biorxiv",
    "bioarxiv",
    "bio archive",
    "medrxiv",
...
PREPRINT_DOI_PREFIXES = [
    "10.1101/",  # classic bioRxiv/medRxiv DOI prefix
    "10.64898/", # CSHL preprint DOI pattern seen on some PMC records
]
...
def is_preprint_article(metadata: dict) -> bool:
    """Return True for bioRxiv/medRxiv-style preprint records.

    The check is intentionally metadata-based and runs before any full-text
    download. This prevents non-peer-reviewed preprints from entering the
    Article table/KG pipeline even if PMC exposes a PDF or abstract XML.
    """
    doi = (metadata.get("doi", "") or "").strip().lower()
    haystack = " ".join([
        metadata.get("title", "") or "",
        metadata.get("journal", "") or "",
        metadata.get("article_type_attr", "") or "",
        " ".join(metadata.get("publication_types", []) or []),
        doi,
    ]).lower()

    if any(pattern in haystack for pattern in PREPRINT_SOURCE_PATTERNS):
        return True

    if "preprint" in haystack and ("rxiv" in haystack or "archive" in haystack):
        return True

    return any(doi.startswith(prefix) for prefix in PREPRINT_DOI_PREFIXES)


def mark_existing_article_as_excluded_preprint(existing: Optional[Article], metadata: dict, db_alias: str) -> None:
    """If a preprint row already exists from an older run, make it ineligible.

    We do not create new rows for preprints. For existing rows, clearing the
    full-text paths and setting type='Preprint' and ds='No' prevents KG
    extraction from selecting the article later.
    """
    if existing is None:
        return

    defaults = {
        "pmid": metadata.get("pmid", 0),
        "title": metadata.get("title", ""),
        "authors": metadata.get("authors", ""),
        "first_author": metadata.get("first_author", ""),
        "journal": metadata.get("journal", ""),
        "year": metadata.get("year", 1900),
        "date": metadata.get("date", date(1900, 1, 1)),
    ...
    Article.objects.using(db_alias).filter(pmcid=metadata.get("pmcid")).update(**defaults)


def _is_preprint_pdf_url(url: str) -> bool:
    lowered = (url or "").lower()
    return any(pattern in lowered for pattern in PREPRINT_SOURCE_PATTERNS)
...
def get_pmc_article_page_url(pmcid: int) -> str:
    """Return the canonical PMC article page URL for a numeric PMCID."""
    return f"https://pmc.ncbi.nlm.nih.gov/articles/PMC{pmcid}/"

def _add_pdf_candidate(urls: List[str], article_url: str, href: str) -> None:
    """Normalize and store a candidate PDF URL discovered from HTML."""
    href = (href or "").strip()
    if not href:
        return

    # Decode common HTML escaping seen in embedded JSON or attributes.
    href = href.replace("\\/", "/").replace("&amp;", "&")

    # Ignore non-download pseudo-links.
    ...
    urls.append(urljoin(article_url, href))


def get_pdf_urls_from_article_page(pmcid: int) -> List[str]:
    """
    Scrape the PMC article webpage for PDF link candidates.

    This is intentionally broader than the old implementation because some
    PMC/preprint records expose PDF links as relative paths such as
    ``pdf/file.pdf`` or as publisher links such as bioRxiv ``.full.pdf``.
    """
    ...
    # 1) Citation/meta PDF URLs.
    for meta in soup.find_all("meta"):
        name = (meta.get("name") or meta.get("property") or "").lower()
        content = (meta.get("content") or "").strip()
        if content and ("pdf" in name or content.lower().endswith(".pdf")):
            _add_pdf_candidate(urls, article_url, content)

    # 2) Normal anchors/buttons. Includes relative paths like ``pdf/foo.pdf``
    # and external publisher links like bioRxiv ``*.full.pdf``.
    for a in soup.find_all("a", href=True):
        ...
        if (
            ...
            _add_pdf_candidate(urls, article_url, href)

    # 3) Data attributes used by some page templates.
    for tag in soup.find_all(True):
        for attr_name, attr_value in tag.attrs.items():
            ...
            if "pdf" in attr_name.lower() or ".pdf" in attr_text.lower() or "/pdf/" in attr_text.lower():
                _add_pdf_candidate(urls, article_url, attr_text)

    # 4) Regex fallback over raw HTML catches escaped links inside scripts/JSON
    # that BeautifulSoup does not expose as regular anchors.
    pdf_patterns = [
        r'''["'](?P<href>https?://[^"']+?\.pdf(?:\?[^"']*)?)["']''',
        r'''["'](?P<href>/[^"']+?\.pdf(?:\?[^"']*)?)["']''',
        r'''["'](?P<href>pdf/[^"']+?\.pdf(?:\?[^"']*)?)["']''',
        r'''(?P<href>https?://[^\s"'<>]+?\.full\.pdf(?:\?[^\s"'<>]*)?)''',
    ]
    for pattern in pdf_patterns:
        for match in re.finditer(pattern, html, flags=re.IGNORECASE):
            _add_pdf_candidate(urls, article_url, match.group("href"))

    # Prefer PMC-hosted PDFs first, then publisher PDFs, while de-duplicating.
    seen = set()
    deduped: List[str] = []
    for url in urls:
    ...
    def priority(url: str) -> int:
        lowered = url.lower()
        if "pmc.ncbi.nlm.nih.gov" in lowered and "/pdf/" in lowered:
            return 0
        if lowered.endswith(".pdf") or ".pdf?" in lowered:
            return 1
        return 2

    return sorted(deduped, key=priority)


def get_pdf_url_from_article_page(pmcid: int) -> Optional[str]:
    """Backward-compatible wrapper returning the first scraped PDF URL."""
    urls = get_pdf_urls_from_article_page(pmcid)
...
def download_pdf_if_available(pmcid: int) -> str:
    ...
    1) OA service PDF
    2) PDF links discovered on the PMC article page, including relative PMC
       links and external publisher/preprint full-PDF links.

    Important: do not fail silently. If all strategies fail, print the reason
    so the PMC sync/server log can show whether the problem was URL discovery,
    a 403/HTML challenge, or a non-PDF response.
    """
    candidate_sources: List[Tuple[str, Optional[str]]] = []

    try:
        candidate_sources.append(("oa_service", get_pdf_url_from_oa_service(pmcid)))
    except Exception as exc:
        print(f"[PMC PDF] pmcid={pmcid} OA service lookup failed: {type(exc).__name__}: {exc}")

    try:
        for url in get_pdf_urls_from_article_page(pmcid):
            candidate_sources.append(("article_page", url))
    except Exception as exc:
        print(f"[PMC PDF] pmcid={pmcid} article-page lookup failed: {type(exc).__name__}: {exc}")

    seen = set()
    for source_name, pdf_url in candidate_sources:
        if not pdf_url:
            continue
        if pdf_url in seen:
            continue
        ...
        if _is_preprint_pdf_url(pdf_url):
            print(f"[PMC PDF] pmcid={pmcid} skipping preprint PDF URL from {source_name}: {pdf_url}")
            continue
        ...
        try:
            response = requests.get(
                ...
                headers={
                    ...
                    "Referer": get_pmc_article_page_url(pmcid),
                },
                timeout=REQUEST_TIMEOUT,
                allow_redirects=True,
            )
            ...
            if response.status_code == 404:
                print(f"[PMC PDF] pmcid={pmcid} {source_name} URL returned 404: {pdf_url}")
                continue
            ...
            if not _looks_like_pdf(response):
                ...
                print(
                    f"[PMC PDF] pmcid={pmcid} {source_name} URL did not return a PDF. "
                    f"content_type={ctype!r} first_bytes={preview!r} url={pdf_url}"
                )
                continue

            pdf_path = _media_path(PDF_SUBDIR, f"{pmcid}.pdf")
            pdf_path.write_bytes(response.content)
            print(f"[PMC PDF] pmcid={pmcid} downloaded PDF from {source_name}: {pdf_url}")
            return _relpath_under_media(pdf_path)

        except requests.RequestException as exc:
            print(f"[PMC PDF] pmcid={pmcid} {source_name} download failed: {type(exc).__name__}: {exc} url={pdf_url}")
            continue

    print(f"[PMC PDF] pmcid={pmcid} no downloadable PDF found")
    return ""
...
def get_article_fulltext_text(article) -> str:
    rel_path = getattr(article, "fulltext_path", "") or ""
    if not rel_path:
        return ""

    abs_path = abs_media_path(rel_path)
    if not os.path.exists(abs_path):
        return ""

    fulltext_format = (getattr(article, "fulltext_format", "") or "").lower()
    if fulltext_format == "pdf" or rel_path.lower().endswith(".pdf"):
        try:
    ...
    try:
        return jats_xml_to_text(abs_path, body_only=True)
    except Exception:
        return ""
...
def ingest_article(
    ...
    if is_preprint_article(metadata):
        mark_existing_article_as_excluded_preprint(existing, metadata, db_alias)
        print(
            f"[PMC INGEST] pmcid={pmcid} skipped preprint source "
            f"journal={metadata.get('journal', '')!r} doi={metadata.get('doi', '')!r}"
        )
        return IngestResult(
            pmcid=pmcid,
            created=False,
    ...
    # PDF-first full-text strategy. EFetch can return only abstract-level XML
    # for some PMC records, so try PDF before downloading any XML full text.
    pdf_rel_path = download_pdf_if_available(pmcid) if download_pdf else ""
    xml_rel_path = ""
    if not pdf_rel_path:
        xml_rel_path = download_fulltext_xml(pmcid)

    # Decide whether the XML is real full-text or only abstract-level XML.
    xml_is_usable_fulltext = False
    xml_abs_path = ""
    if xml_rel_path:
        xml_abs_path = abs_media_path(xml_rel_path)
        if os.path.exists(xml_abs_path):
            xml_is_usable_fulltext = xml_has_usable_fulltext(Path(xml_abs_path))
            if not xml_is_usable_fulltext:
                # Do not keep abstract-only EFetch XML as a successful full-text
                # artifact. It misleads the UI and causes zero-triple extraction.
                try:
                    Path(xml_abs_path).unlink(missing_ok=True)
                except Exception:
                    pass
                print(f"[PMC XML] pmcid={pmcid} XML was abstract-only; not using it as full text")
                xml_rel_path = ""
                xml_abs_path = ""

    if not pdf_rel_path and not xml_is_usable_fulltext:
        raise PMCPipelineError(
            f"No peer-reviewed full text found for PMCID {pmcid}: "
            "PDF was unavailable and XML was missing or abstract-only"
        )

    # Use the best available full text for classification:
    # 1) full-body XML, 2) PDF text.
    fulltext_text = ""
    if xml_is_usable_fulltext and xml_abs_path:
        try:
            fulltext_text = jats_xml_to_text(xml_abs_path, body_only=True)
        except Exception:
            fulltext_text = ""

    if not fulltext_text and pdf_rel_path:
        try:
            pdf_abs_path = abs_media_path(pdf_rel_path)
            if os.path.exists(pdf_abs_path):
                fulltext_text = extract_text_from_pdf(pdf_abs_path)
        except Exception:
            fulltext_text = ""

    if not fulltext_text and xml_abs_path:
        try:
            fulltext_text = jats_xml_to_text(xml_abs_path, body_only=False)
        except Exception:
            fulltext_text = ""
    ...
    # Store the real full-text source in fulltext_path. If XML is abstract-only
    # but a PDF was downloaded, make the PDF the effective full text so the UI
    # and downstream pipeline do not treat the abstract XML as the article body.
    effective_fulltext_path = ""
    effective_fulltext_format = ""
    if xml_is_usable_fulltext and xml_rel_path:
        effective_fulltext_path = xml_rel_path
        effective_fulltext_format = "xml"
    elif pdf_rel_path:
        effective_fulltext_path = pdf_rel_path
        effective_fulltext_format = "pdf"
    ...
    defaults = {
        ...
        "fulltext_path": effective_fulltext_path,
        "fulltext_format": effective_fulltext_format,
    ...
    return IngestResult(
        ...
        has_xml=bool(xml_is_usable_fulltext and xml_rel_path),
...
def ingest_search_results(
    ...
    for pmcid in pmcids:
        try:
            ...
            ingested.append({
                ...
                "skipped_preprint": result.skipped_preprint,
    ...
    return {
        ...
        "counts": {
            ...
            "skipped_preprints": skipped_preprints,
            "errors": len(errors),
        },
        "items": ingested,
        "errors": errors,
